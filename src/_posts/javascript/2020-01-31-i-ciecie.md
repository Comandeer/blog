...
{% note %}`script[type=module]` powoduje, że cały kod JS zawarty w tym elemencie jest uruchamiany w strict mode. Osobiście uważam to za bardziej eleganckie rozwiązanie niż `'use strict';`.{% endnote %}
...
{% note %}Więcej o nowej składni bloków `try`/`catch` przeczytać można na [MDN](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/try...catch#The_exception_identifier).{% endnote %}
