...
{% note %}Instrukcja posiada wewnątrz sobie średnik, bo [zgodnie ze specyfikacją ECMAScript jest to znak kończący instrukcję](https://tc39.github.io/ecma262/#prod-ExpressionStatement). Teoretycznie kod bez średnika na końcu byłby niepoprawny składniowo, gdyby nie [mechanizm automatycznego wstawiania średników](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Lexical_grammar#Automatic_semicolon_insertion).{% endnote %}
...
{% note %}Jeśli zastanawiasz się, czemu domyślny eksport modułu wymaga odwołania się bezpośrednio do `default`, to jest to [efekt transpilacji modułu ES do formatu CJS, z zachowaniem semantyki opisanej w specyfikacji ECMAScript](http://2ality.com/2017/01/babel-esm-spec-mode.html).{% endnote %}
...
{% note %}Niestety, nigdzie nie udało mi się znaleźć dokumentacji obiektu `path`. Najbliżej tego znajduje się [Babel Plugin Handbook](https://github.com/jamiebuilds/babel-handbook/blob/master/translations/en/plugin-handbook.md).{% endnote %}
