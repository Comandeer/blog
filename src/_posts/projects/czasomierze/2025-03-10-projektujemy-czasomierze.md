...
{% note %}Część przykładów z tego artykułu może nie chcieć się uruchomić we wspomnianym wyżej narzędziu, narzekając na obecność [top-level `await`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/await#top_level_await). W takim wypadku wystarczy wymusić traktowanie kodu jako modułu ES poprzez dodanie, czy to na początku, czy na końcu, pustego eksportu (`export {}`) lub otoczyć całość w [samowywołującą się asynchroniczną funkcję](https://developer.mozilla.org/en-US/docs/Glossary/IIFE).{% endnote %}
...
{% note %}Możemy podać dokładny powód przerwania operacji, przekazując go jako argument do `AbortController#abort()`. Można tam przekazać dowolną wartość, ale przekazywanie [błędów](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Error) w takich przypadkach jest w miarę powszechnie stosowaną dobrą praktyką.{% endnote %}
...
{% note %}Aktualny znacznik czasu niekoniecznie jest szczególnie przydatny w przypadku pojedynczego wywołania `setTimeout()`, ale w przypadku `setInterval()` może być przydatny do wykorzystania w technice [<i lang="en">delta timing</i>](https://en.wikipedia.org/wiki/Delta_timing).{% endnote %}
